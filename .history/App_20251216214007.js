...
import {
  ...
  Keyboard, Alert, StatusBar, Platform, FlatList, Dimensions
} from 'react-native';
...
// ==========================================
// 1. ISOLATED SEARCH BAR (Handles its own state)
// ==========================================
const SearchModule = ({ onCitySelect }) => {
  const [query, setQuery] = useState('');
  const [suggestions, setSuggestions] = useState([]);
  const [showList, setShowList] = useState(false);

  // Debounce Logic: Only search API 400ms after user STOPS typing
  useEffect(() => {
  ...
  return (
    <View style={styles.searchContainer}>
      ...
      {/* Suggestions List - Rendered only when needed */}
...
// ==========================================
// 2. MEMOIZED WEATHER DISPLAY (Doesn't re-render on typing)
// ==========================================
...
// ==========================================
// 3. MEMOIZED LIST ITEM (Optimized Rendering)
// ==========================================
...
export default function App() {
  ...
  return (
    <LinearGradient colors={weatherProps.bg} style={styles.container}>
      ...
      {/* Search is now Isolated - Typing won't lag the app */}
      <View style={styles.topSpacer} />
      <SearchModule onCitySelect={(item) => fetchWeather(item.latitude, item.longitude, `${item.name}, ${item.country}`)} />
      ...
      {loading ? (
        ...
        <FlatList
          ...
          initialNumToRender={7} // Performance: Only render visible items
        />
      ) : null}
    </LinearGradient>
  );
...
const styles = StyleSheet.create({
  ...
  // Search Styles
  searchContainer: { marginHorizontal: 20, zIndex: 100, marginBottom: 10 },
  searchBarGlass: {
