...
export default function App() {
  ...
  const [timezoneOffset, setTimezoneOffset] = useState(0); // OWM returns offset in seconds
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      // 3. Fetch Air Quality (Separate API in OWM)
      const aqiRes = await fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${API_KEY}`);
      const aqiData = await aqiRes.json();

      if (currentData.cod !== 200 || forecastData.cod !== "200") throw new Error("API Error");

      // Update Timezone Offset
      setTimezoneOffset(currentData.timezone); // Seconds from UTC
      ...
      else if (codeId >= 200 && codeId < 600) setBgImage(BACKGROUNDS.RAIN); // Includes Thunder/Drizzle
      ...
      // --- PROCESS HOURLY DATA (Next 24h from 3-hour chunks) ---
      // OWM Free only gives 3-hour steps, so we map those
      const hourly = forecastData.list.slice(0, 8).map(item => ({
        key: item.dt.toString(),
        time: new Date((item.dt + currentData.timezone) * 1000).toISOString().substr(11, 5), // quick UTC shift hack or just use local format below
        temp: Math.round(item.main.temp),
        code: mapOWMCode(item.weather[0].id),
        windSpeed: Math.round(item.wind.speed * 3.6), // m/s to km/h
        windDir: item.wind.deg,
        // Since we want accurate local time string manually shifted:
        localTimeObj: new Date((item.dt + currentData.timezone) * 1000)
      }));

      // Fix Hourly Time Display (Manual Offset Calculation)
      // OWM returns UTC unix timestamps. We add the offset manually for display.
      const processedHourly = hourly.map(h => {
        // We actually just want to format the time relative to the city timezone.
        // But simply:
      ...
      // --- PROCESS DAILY DATA (Aggregate 3-hour chunks into days) ---
      const dailyMap = {};
      forecastData.list.forEach(item => {
        const date = new Date((item.dt + currentData.timezone) * 1000).toISOString().split('T')[0];
        if (!dailyMap[date]) {
          dailyMap[date] = {
            date: new Date(item.dt * 1000), // Keep original object for date formatting
            temps: [],
            codes: [],
            windSpeeds: [],
            windDirs: [],
      ...
      const daily = Object.values(dailyMap).slice(0, 5).map((d, i) => ({
        key: i.toString(),
        date: d.date, // This needs to be a Date object
        maxTemp: Math.max(...d.temps),
        minTemp: Math.min(...d.temps),
        code: mapOWMCode(d.codes[Math.floor(d.codes.length / 2)]), // Take midday code
        apparentMax: Math.max(...d.feels_like),
        windSpeed: Math.round(Math.max(...d.windSpeeds) * 3.6),
        windDir: d.windDirs[0], // Approximate
      }));
      ...
      setWeatherData({
        ...
        extra: {
          ...
          aqi: aqiData.list[0].main.aqi * 50, // Approximate conversion 1-5 to US AQI scale
        },
        daily: daily
      });

      // If searching, update city name from API result
      if (isManualSearch.current) {
        setCity(`${currentData.name}, ${currentData.sys.country}`);
      }

    } catch (e) {
      if (!isSilent) Alert.alert("Error", "Could not fetch weather. Check API Key.");
    } finally {
      if (!isSilent) setLoading(false);
    }
  };
  ...
  // LIVE CLOCK: Manually calculate city time using the offset
  // 'now' is UTC device time. We add the city offset (seconds) * 1000
  // Note: This simple calculation assumes 'now' is correct UTC. 
  // A safer way for display is getting UTC time + offset.
