...
function MainScreen() {
  ...
  const getCurrentLocation = async () => {
    ...
    try {
      ...
      // 1. FAST: Try to get last known location first (Instant)
      let loc = await Location.getLastKnownPositionAsync({});

      if (loc) {
        // If we found a cached location, use it immediately!
        fetchWeather(loc.coords.latitude, loc.coords.longitude, 'My Location');
      } else {
        // 2. SLOW: If no cached location, wait for fresh GPS signal
  ...
  const fetchWeather = async (lat, lon, cityName) => {
    // Don't set loading true if we are just refreshing data invisibly
    // But here we want to show loading for the weather fetch itself
