...
export default function App() {
  ...
  // We use a REF to track the current location parameters for the auto-refresh
  // This prevents stale closures in the setInterval
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (lastParams.current) {
        // Silent update (pass true as 4th argument)
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    // Save these params for the auto-refresher
    lastParams.current = { lat, lon, cityName };

    setCity(cityName);
    // Only show the loading spinner if it's NOT a silent update and we have no data
    ...
    try {
      ...
      setWeatherData({
        ...
        // --- 24-HOUR FORECAST PROCESSING ---
        hourly: data.hourly.time.map((time, i) => ({
          key: time,
          // UPDATED: 'en-GB' forces 24-hour format (e.g., 23:00)
