...
// Import our files
import { BACKGROUNDS, getIconProps, getTempColor } from './src/helpers';
    ...
    <View style={styles.graphContainer}>
      ...
      <View style={styles.graphBody}>
        {data.map((item, index) => {
          ...
          const barColor = getTempColor(item.maxTemp); // Get Color

          return (
            <View key={index} style={styles.graphBarWrapper}>
              <Text style={styles.graphTempText}>{Math.round(item.maxTemp)}°</Text>
              {/* Apply Color to Bar */}
              <View style={[styles.graphBar, { height: `${Math.max(heightPercent, 10)}%`, backgroundColor: barColor }]} />
    ...
    try {
      ...
      setWeatherData({
        ...
        hourly: data.hourly.time.map((time, i) => ({
          key: time,
          time: new Date(time).toLocaleTimeString('en-US', { timeZone: data.timezone, hour: 'numeric', hour12: false }), // 24h format for graph
