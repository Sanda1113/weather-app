...
export default function App() {
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      // Process Hourly (using UTC + Timezone offset for correct local hours)
      const hourly = forecastData.list.slice(0, 8).map(item => {
        // Correctly offset the time
        const utcDate = new Date(item.dt * 1000);
        const localDate = new Date(utcDate.getTime() + (currentData.timezone * 1000));

        // We need to trick the Date object to display the "City Time" even if user is in different timezone
        // The easiest way for display purposes is getUTCHours since we shifted it by offset
        ...
        return {
          ...
          windSpeed: (item.wind.speed * 3.6).toFixed(1), // show decimal for wind like in screenshot
      ...
      if (!isSilent) Alert.alert("Connection Error", "Please check your internet connection.");
    } finally {
      if (!isSilent) setLoading(false);
    }
  };
