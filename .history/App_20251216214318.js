...
import {
  ...
  Keyboard, Alert, StatusBar, Platform, FlatList, TouchableWithoutFeedback
} from 'react-native';
...
// ==========================================
// 1. SEARCH MODULE (With Spinner & Auto-Close)
// ==========================================
const SearchModule = ({ onCitySelect, isVisible, setIsVisible }) => {
  const [query, setQuery] = useState('');
  const [suggestions, setSuggestions] = useState([]);
  const [isSearching, setIsSearching] = useState(false); // Local loading state
  ...
  useEffect(() => {
    ...
    setIsSearching(true); // Start spinner immediately on type
  ...
  return (
    <View style={styles.searchContainer}>
      <View style={styles.searchBarGlass}>
        ...
        {/* RIGHT SIDE ICONS: Loading Spinner OR Close Button */}
        {isSearching ? (
...
// ==========================================
// 2. WEATHER COMPONENTS (Optimized)
// ==========================================
...
export default function App() {
  ...
  const [searchVisible, setSearchVisible] = useState(false); // Lifted state to control visibility
  ...
  // Logic to dismiss search when tapping outside
  const dismissSearch = () => {
    if (searchVisible) {
  ...
  return (
    <LinearGradient colors={weatherProps.bg} style={styles.container}>
      ...
      {/* SEARCH MODULE */}
      {/* We pass the visibility state down so SearchModule can use it */}
      ...
      {/* TOUCHABLE AREA TO CLOSE SEARCH */}
      {/* We wrap the content in a touchable area to detect clicks outside */}
      <View style={{ flex: 1 }} onStartShouldSetResponder={() => true} onResponderRelease={dismissSearch}>
        {loading ? (
          ...
          <FlatList
            ...
            keyboardShouldPersistTaps="handled" // Important so list taps don't just close keyboard
          />
        ) : null}
      </View>
    </LinearGradient>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  topSpacer: { height: Platform.OS === 'ios' ? 60 : 40 },
  ...
  cityName: { fontSize: 32, fontWeight: '700', color: '#fff', textAlign: 'center' },
  date: { fontSize: 14, color: '#ddd', marginTop: 5 },
  heroIcon: { marginVertical: 20 },
  temp: { fontSize: 90, fontWeight: '300', color: '#fff' },
  condition: { fontSize: 24, fontWeight: '500', color: '#eee', marginBottom: 20 },
  statsGlass: {
  ...
  statLabel: { color: '#aaa', fontSize: 12, marginTop: 5 },
  ...
  dayName: { color: '#fff', fontSize: 16, width: 60, fontWeight: '600' },
  tempHighLow: { flexDirection: 'row', width: 80, justifyContent: 'flex-end', gap: 10 },
  maxTemp: { color: '#fff', fontWeight: '700' },
  minTemp: { color: '#aaa' },
});