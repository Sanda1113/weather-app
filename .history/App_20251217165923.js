...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Refresh Weather every 20 seconds (10s is too fast for OWM Free Tier sometimes)
  ...
  const getFullUpdate = async (isSilent = false) => {
    ...
    try {
      ...
      // Request HIGH accuracy to fix location issues
  ...
  const mapOWMCode = (id) => {
    ...
    if (id >= 300 && id < 600) return 61; // Rain (Drizzle or Rain)
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      if (currentRes.status !== 200) throw new Error("API Connection Error");
      ...
      // --- LIVE ACCURACY FIX ---
      // 1. Get the standard ID
      let codeId = currentData.weather[0].id;

      // 2. Check for ACTUAL Rain Volume (The "Cloudy but Raining" fix)
      // OWM puts rain volume in currentData.rain (e.g., currentData.rain['1h'])
      if (currentData.rain && (currentData.rain['1h'] > 0 || currentData.rain['3h'] > 0)) {
        // If the ID says Clouds (80x) but there is rain volume, FORCE it to be Rain
        if (codeId >= 800) {
          codeId = 500; // Force Rain ID
        }
      }
      ...
      const daily = Object.values(dailyMap).slice(0, 5).map((d, i) => {
        ...
        // Instead of taking the middle code, check if it rains AT ALL during the day
        let worstCode = d.codes[Math.floor(d.codes.length / 2)]; // Default to midday

        // Scan for Rain/Thunder to override "Cloudy"
        if (d.codes.some(c => c >= 200 && c < 300)) worstCode = 200; // Thunder priority
        else if (d.codes.some(c => c >= 600 && c < 700)) worstCode = 600; // Snow priority
        else if (d.codes.some(c => c >= 300 && c < 600)) worstCode = 500; // Rain priority
        ...
        return {
          ...
          code: mapOWMCode(worstCode), // Use the "Worst" code found
