...
export default function App() {
  ...
  // --- NEW: PRIORITY CHECKER ---
  // Returns the most "Severe" weather object from the list
  const getPriorityWeather = (weatherList) => {
    if (!weatherList || weatherList.length === 0) return { id: 800, main: 'Clear' };

    // Order of importance (Lower index = Higher priority to display)
    // 2xx (Thunder) > 6xx (Snow) > 5xx (Rain) > 3xx (Drizzle) > 7xx (Fog) > 80x (Clouds) > 800 (Clear)
    const severity = (id) => {
    ...
    return weatherList.reduce((prev, current) => {
      return severity(current.id) > severity(prev.id) ? current : prev;
    });
  };
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      // --- ACCURACY FIX: Pick the worst condition ---
      const primaryWeather = getPriorityWeather(currentData.weather);
      const codeId = primaryWeather.id;
      const isNight = primaryWeather.icon.includes('n');
      ...
      const hourly = forecastData.list.slice(0, 8).map(item => {
        ...
        // Also prioritize hourly weather (sometimes 3-hour forecast has mixed states)
        const hourlyPrimary = getPriorityWeather(item.weather);

        return {
          key: item.dt.toString(),
          time: hourStr,
          temp: Math.round(item.main.temp),
          code: mapOWMCode(hourlyPrimary.id),
      ...
      forecastData.list.forEach(item => {
        ...
        // Push the PRIORITY code for this 3-hour block, not just the first one
        const blockPriority = getPriorityWeather(item.weather);
        dailyMap[date].codes.push(blockPriority.id);
      ...
      const daily = Object.values(dailyMap).slice(0, 5).map((d, i) => {
        // Find worst code in the whole day
        let worstCodeId = 800;
        let maxSeverity = 0;

        // Re-use logic to find worst daily condition
        const severity = (id) => {
          if (id >= 200 && id < 300) return 10;
          if (id >= 600 && id < 700) return 9;
          if (id >= 500 && id < 600) return 8;
