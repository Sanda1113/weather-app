...
export default function App() {
  ...
  const mapOWMCode = (id) => {
    // Universal Mapping for "Every Single Weather"
    ...
    if (id >= 700 && id < 800) return 45; // Fog/Mist/Haze/Dust/Sand/Ash
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      // --- LOGIC 1: UNIVERSAL PRECIPITATION CHECK ---
      let codeId = currentData.weather[0].id;
      const temp = currentData.main.temp;

      // Check Probability of Precipitation (pop) in immediate forecast
      const immediateForecast = forecastData.list[0];
      const rainProbability = immediateForecast.pop; // 0.0 to 1.0

      // If it says "Clear" or "Cloudy" BUT there is >20% chance of precipitation...
      if (codeId >= 800 && rainProbability > 0.2) {
        if (temp < 0) {
          codeId = 600; // Force SNOW if freezing
        } else {
          codeId = 500; // Force RAIN if warm
        }
      }

      // --- LOGIC 2: SEVERITY PRIORITY CHECK ---
      // If API sends multiple conditions (e.g. "Mist" + "Clear"), show the worst one.
      if (currentData.weather.length > 1) {
        // Priority: Thunder > Snow > Rain > Fog > Drizzle > Clouds > Clear
        const getSeverity = (id) => {
          if (id >= 200 && id < 300) return 100; // Thunder
          if (id >= 600 && id < 700) return 90;  // Snow
          if (id >= 500 && id < 600) return 80;  // Rain
          if (id >= 700 && id < 800) return 70;  // Fog/Dust
          if (id >= 300 && id < 400) return 60;  // Drizzle
          if (id > 800) return 10;               // Clouds
          return 0;                              // Clear
        };

        // Sort by severity and pick the top one
      ...
      else if (codeId >= 700 && codeId < 800) setBgImage(BACKGROUNDS.CLOUDY); // Fog looks better with cloudy BG usually
      ...
      const daily = Object.values(dailyMap).slice(0, 5).map((d, i) => {
        ...
        // Severity Logic for Daily Summary
        const severity = (id) => {
        ...
        // 1. Find the worst reported code
        d.codes.forEach(c => {
        ...
        // 2. Check "Probability of Precipitation" for the day
        const highRainChance = d.pops.some(p => p > 0.35); // If >35% chance at any point
        if (maxSeverity < 60 && highRainChance) {
          // If we missed the rain code but probability is high, assume Rain or Snow
