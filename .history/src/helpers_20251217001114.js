...
export const STATS_COLORS = {
    WIND: '#E0E0E0',      // Light Gray (Standard for Wind/Air)
    HUMIDITY: '#00BFFF',  // Deep Sky Blue (Standard for Water)
};

export const getIconProps = (code) => {
    if (code === 0) return { name: 'weather-sunny', color: '#FFD700', label: 'Sunny' };
    // CHANGED: Clouds are now a standard Light Steel Blue/Grey instead of Neon
    if (code >= 1 && code <= 3) return { name: 'weather-partly-cloudy', color: '#B0C4DE', label: 'Cloudy' };
    if (code >= 45 && code <= 48) return { name: 'weather-fog', color: '#CFD8DC', label: 'Foggy' };
    ...
    if (code >= 95) return { name: 'weather-lightning', color: '#FFCC00', label: 'Storm' };
...
export const getTempColor = (t) => {
    ...
    if (t >= 10 && t < 18) return '#4CAF50'; // Mild Green
    if (t >= 18 && t < 25) return '#CDDC39'; // Warm Yellow-Green
    if (t >= 25 && t < 32) return '#FF9800'; // Hot Orange
    if (t >= 32 && t < 40) return '#F44336'; // Very Hot Red
    return '#B71C1C'; // Extreme
};