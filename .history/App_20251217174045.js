...
export default function App() {
  ...
  // --- COMPREHENSIVE MAPPING FOR WEATHERAPI.COM CODES ---
  const mapWeatherAPICode = (code) => {
    ...
    // RAIN / DRIZZLE
    // 1063=Patchy rain, 1150-1153=Drizzle, 1180-1195=Rain, 1240-1246=Rain Showers
    if ([1063, 1150, 1153, 1180, 1183, 1186, 1189, 1192, 1195, 1240, 1243, 1246].includes(code)) return 61;

    // SNOW / ICE / SLEET
    // 1066=Patchy snow, 1069=Sleet, 1072=Freezing drizzle
    // 1114=Blowing snow, 1117=Blizzard, 1204-1207=Sleet, 1210-1225=Snow
    // 1237=Ice pellets, 1249-1252=Sleet showers, 1255-1258=Snow showers, 1261-1264=Ice pellets
    // 1168-1171=Freezing drizzle, 1198-1201=Freezing rain
    if ([1066, 1069, 1072, 1114, 1117, 1168, 1171, 1198, 1201, 1204, 1207, 1210, 1213, 1216, 1219, 1222, 1225, 1237, 1249, 1252, 1255, 1258, 1261, 1264].includes(code)) return 71;
  ...
  const fetchWeather = async (lat, lon, cityName, isSilent = false) => {
    ...
    try {
      ...
      const mappedCode = mapWeatherAPICode(code);

      // FORCE BACKGROUND UPDATE BASED ON MAPPED CODE
      ...
      else if (mappedCode === 45) setBgImage(BACKGROUNDS.CLOUDY); // Fog looks best with Cloudy BG
      ...
      const daily = forecast.map((d, i) => ({
        ...
        code: mapWeatherAPICode(d.day.condition.code),
        apparentMax: Math.round(d.day.avgtemp_c),
      ...
      setWeatherData({
        ...
        extra: {
          ...
          aqi: Math.round(current.air_quality['us-epa-index'] * 50) || 0,
        },
        daily: daily,
