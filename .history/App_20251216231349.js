...
const getIconProps = (code) => {
  // Sunny / Clear
  if (code === 0) return { name: 'weather-sunny', color: '#FFD700', label: 'Sunny' }; // Gold
  // Partly Cloudy
  if (code >= 1 && code <= 3) return { name: 'weather-partly-cloudy', color: '#B0C4DE', label: 'Cloudy' }; // LightSteelBlue
  // Fog
  if (code >= 45 && code <= 48) return { name: 'weather-fog', color: '#dcdcdc', label: 'Foggy' }; // Gainsboro
  // Rain / Drizzle
  if (code >= 51 && code <= 67) return { name: 'weather-rainy', color: '#00BFFF', label: 'Rainy' }; // DeepSkyBlue
  // Snow
  if (code >= 71 && code <= 77) return { name: 'weather-snowy', color: '#00FFFF', label: 'Snowy' }; // Cyan
  // Thunderstorm
  if (code >= 95) return { name: 'weather-lightning', color: '#FFA500', label: 'Storm' }; // Orange
  // Default
  return { name: 'weather-cloudy', color: '#fff', label: 'Overcast' };
};
...
function MainScreen() {
  ...
  const fetchWeather = async (lat, lon, cityName) => {
    ...
    } catch (e) { if (!weatherData) setError(true); } finally { setLoading(false); }
  };
        ...
        <View style={{ flex: 1 }}>
          {loading ? (
            ...
            <FlatList
              ...
              ListFooterComponent={
                // EXPANSION BUTTON - STYLED TO MATCH
...
const styles = StyleSheet.create({
  ...
  // Expansion Button
  fullForecastBtn: {
    backgroundColor: 'rgba(0,0,0,0.3)', // Same as row
    marginHorizontal: 20, marginTop: 0, padding: 12,
    borderBottomLeftRadius: 15, borderBottomRightRadius: 15,
    borderTopLeftRadius: 5, borderTopRightRadius: 5, // Tweak to look attached
    flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 5,
  },
  fullForecastText: { color: '#ddd', fontSize: 14, fontWeight: '500' },
